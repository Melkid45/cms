...
function resolveFrom(file, request) {
  ...
  if (normalizedRequest === '@') return srcDir
  if (normalizedRequest.startsWith('@/')) return path.resolve(srcDir, normalizedRequest.slice(2))

  const relativePath = path.resolve(path.dirname(file), normalizedRequest)
  return existsSync(relativePath) ? relativePath : path.resolve(root, normalizedRequest)
}

function resolvePublicPath(request, file) {
  if (/^(?:\/|[a-z]+:|#)/i.test(request)) return request
...
function rewriteAliasAttributes(html, file) {
  ...
  return html
    .replace(attributePattern, (_, attribute, request) => {
      return `${attribute}="${resolvePublicPath(request, file)}"`
    })
...
function extractStyleLinks(html, file, styles) {
  return html.replace(
    /<link\b[^>]*\bhref=["']([^"']+\.(?:css|s[ac]ss))["'][^>]*>\s*/gi,
    (tag, href) => {
      if (/^[a-z]+:/i.test(href)) return tag
      styles.add(resolvePublicPath(href, file))
      return ''
    },
  )
}
...
function renderPage(html, file) {
  ...
  if (!layoutMatch) return injectStyleLinks(pageHtml, pageStyles)
